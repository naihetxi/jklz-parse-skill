...
SOURCE="${SCRIPT_DIR}/cli/build/${TARGET}"
DEST="${INSTALL_DIR}/${EXECUTABLE_NAME}"

echo "Detected platform: ${OS_LOWER}/${ARCH_LOWER}"
echo "Binary file: ${SOURCE}"

if [ ! -f "${SOURCE}" ]; then
    echo "ERROR: binary file not found: ${SOURCE}"
    echo "Please confirm the package contains cli/build/${TARGET}."
    exit 1
fi
...
echo "   jklz-parse config --api-key YOUR_API_KEY --base-url http://YOUR_HOST:PORT"
echo ""
echo "Verify:"
