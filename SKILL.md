...
| 触发条件 | 建议对策 | 仍失败兜底 |
|---------|---------|----------|
| CLI 返回 502/503 | 等待 5s 后重试 1 次 | 尝试降低精度切换 `--image-mode cv` 再试 |
| CLI 返回 401 | 检查 API Key 配置 | 提示用户重新配置 `config --api-key` |
...
| `toc` | 目录结构 | 获取文档目录树 |
| `table` | 表格信息 | 提取所有表格，配合 `--table-format markdown` 食用更佳 |
...
| 提取表格 | `--return table --table-format markdown` | 表格转 Markdown |
| 完整解析 | `--return content#toc#table` | 文本+目录+表格 |
| 性能要求高 | `--image-mode cv` | 速度快，适合大文件 |
...
2. **不要直接硬编码 `curl` 命令调用原始 SSE API**
   - 所有的调用应当且只能通过内置的 `cli/jklz-parse.py` 或是 `cli/build/` 下的可执行文件来完成。这样可以复用 CLI 的异常重试和流式结果合并逻辑。
3. **不要忽略配置报错**
